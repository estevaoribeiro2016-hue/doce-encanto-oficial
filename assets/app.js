...
function renderAdmin(){
 const pending=orders.filter(o=>!orderIsDelivered(o)&&!orderIsCanceled(o)),history=orders.filter(o=>orderIsDelivered(o)||orderIsCanceled(o)),production=pending.filter(orderIsProduction),deliveredRevenue=orders.filter(o=>orderIsDelivered(o)&&!o.isTest).reduce((a,o)=>a+o.total+(o.revenueAdjustment||0),0),low=products.filter(p=>!p.unavailable&&p.stock<=p.min).length;
 $('#adminPanel').innerHTML=`<div class="adminHero"><div><p class="tag">Centro de Controle V55</p><h2>Área da Empresa</h2><p>Pedidos, estoque, financeiro mensal, impressão térmica e taxas de entrega online.</p></div><div class="adminTopActions">${supabaseStatusHtml()}<button id="toggleOrderAlarm" class="secondary">${newOrderAlarmEnabled?'🔔 Alarme ligado':'🔕 Ativar alarme'}</button><button id="adminBack" class="secondary">Voltar ao site</button><button id="adminLogout" class="ghost">Sair</button></div></div><div class="dashCards"><div><small>Pendentes</small><b>${pending.length}</b></div><div><small>Produção</small><b>${production.length}</b></div><div><small>Estoque baixo</small><b>${low}</b></div><div><small>Faturamento entregue</small><b>${BRL(deliveredRevenue)}</b></div></div><div class="adminTabs"><button class="active" data-tabbtn="pending">📌 Pendentes</button><button data-tabbtn="production">🏭 Produção</button><button data-tabbtn="history">📚 Histórico</button><button data-tabbtn="stock">📦 Estoque</button><button data-tabbtn="moves">🔁 Movimentações</button><button data-tabbtn="finance">💰 Financeiro mensal</button><button data-tabbtn="zones">🛵 Taxas</button><button data-tabbtn="settings">⚙️ Configurações</button></div>
 ...
 <div class="tabPanel" data-tab="finance"><section class="adminCard wide"><h3>💰 Faturamento por mês</h3><p class="helper">Somente pedidos entregues e não marcados como teste entram no faturamento. Ajustes manuais ficam registrados.</p>${monthlyFinanceHtml()}<h4>Pedidos entregues e ajustes</h4><div class="historyTable">${orders.filter(orderIsDelivered).map(o=>`<div class="historyRow"><span>#${o.id}${o.isTest?' • TESTE':''}</span><span>${o.customerName}</span><span>${BRL(o.total+(o.revenueAdjustment||0))}</span><span>${currentAdmin==='teteu.trufa'?`<button class="secondary" data-revenue="${o.id}">Corrigir</button>`:''} <button class="ghost" data-test="${o.id}">${o.isTest?'Tornar real':'Marcar teste'}</button></span></div>`).join('')||'<p class="emptyState">Nenhum pedido entregue.</p>'}</div></section></div>
